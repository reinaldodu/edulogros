...
Begin VB.Form PEND_GENRL2 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Total de logros perdidos y reaprendizaje por materia - Mitad de periodo"
   ClientHeight    =   7050
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   9525
   Icon            =   "PEND_GENRL2.frx":0000
   LinkTopic       =   "Form1"
   LockControls    =   -1  'True
   MaxButton       =   0   'False
   ScaleHeight     =   7050
   ...
   Begin VB.ComboBox Combo2 
      Height          =   315
      ItemData        =   "PEND_GENRL2.frx":0442
      Left            =   7680
      List            =   "PEND_GENRL2.frx":0455
...
If Option1.Value = True Then
    TituloPrint = "REPORTE TOTAL DE LOGROS PERDIDOS POR MATERIA"
End If
If Option2.Value = True Then
    TituloPrint = "REPORTE TOTAL DE LOGROS CON REAPRENDIZAJE POR MATERIA"
End If
If Option3.Value = True Then
    TituloPrint = "REPORTE TOTAL DE LOGROS EN PROCESO (LEP) POR MATERIA"
End If
    ...
    While Not EOF(NAR)
        ...
        If RTrim(argra.nom_grup) = Combo1.Text Then
            ...
            For ww = 1 To lw
                ...
                If Option3.Value = True Then
                    If Dir(Ruta & Combo1.Text & argra.num_area & lw & ".obs") <> "" Then
                        'ValiNota = False
                        NAR = FreeFile
                        Open Ruta & Combo1.Text & argra.num_area & lw & ".obs" For Random As #NAR Len = Len(notas)
                        Get #NAR, J, notas
                        Close #NAR
                        NAR = NAR - 1
                        For z = 1 To 10
                            If notas.area(z) <> 0 Then
                                ...
                                Get #NAR, (Val(notas.area(z))), logru
                                Close #NAR
                                NAR = NAR - 1
                                If logru.indicador = "LEP" Then
                                    CP = CP + 1
                                    ValiNota = True
                                End If
                            End If
...
End If
If Option3.Value = True Then
    MATI126.TextMatrix(MATI126.Rows - 1, 1) = "TOTAL LOGROS EN PROCESO (LEP)..."
End If
