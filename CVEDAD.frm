VERSION 5.00
Begin VB.Form CVEDAD 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Consulta por edad"
   ClientHeight    =   1335
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   2910
   Icon            =   "CVEDAD.frx":0000
   LinkTopic       =   "Form1"
   LockControls    =   -1  'True
   MaxButton       =   0   'False
   MinButton       =   0   'False
...
While Not EOF(NAR)
    ...
    If mm = Month(Date) Then
        ...
        End If
    End If
    If (RTrim(alumno.n_carnet) <> "") And (Val(aaaa) = Val(Combo1.Text)) Then
        NAR = FreeFile
...
Close #NAR
If cona = 2 Then
    MsgBox "NO SE ENCONTRARON REGISTROS PARA ESTA EDAD", 64
    Screen.MousePointer = 0
    Exit Sub
End If
CONSVARIAS.Caption = "Consultas opcionales - [total registros encontrados = " & (CONSVARIAS.MATRICON.Rows - 1) & "]"
CONSVARIAS.Frame1.Caption = "CONSULTA POR EDADES (EDAD = " & Combo1.Text & " AÑOS)"
...
ENTRADA.stb.Panels.Item(1).Text = "Consulta de alumnos por edades."
End Sub
