...
While Not EOF(NAR)
    J = J + 1
    Get #NAR, J, alumno
    If (RTrim(alumno.n_carnet) <> "") And (RTrim(alumno.rh) = RTrim(Combo1.Text)) Then
        NAR = FreeFile
...
Close #NAR
If cona = 2 Then
    MsgBox "NO SE ENCONTRARON REGISTROS PARA ESTE R.H.", 64
    Screen.MousePointer = 0
    Exit Sub
End If
CONSVARIAS.Caption = "Consultas opcionales - [total registros encontrados = " & (CONSVARIAS.MATRICON.Rows - 1) & "]"
CONSVARIAS.Frame1.Caption = "CONSULTA POR R.H. (R.H. = " & Combo1.Text & ")"
...
ENTRADA.stb.Panels.Item(1).Text = "Consulta de alumnos por R.H."
End Sub
